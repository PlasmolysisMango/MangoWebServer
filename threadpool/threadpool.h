...
public:
    ...
    enum mode {REACTOR, PROACTOR};
    ThreadPool(mode actor_mode, size_t thread_num = 8, size_t max_requests = 10000);
    ~ThreadPool() {}
    // 增加带状态任务
    bool append(T *request, int stat);
    // 增加任务
    bool append_req(T *request);
...
template<typename T>
ThreadPool<T>::ThreadPool(mode actor_mode, size_t thread_num = 8, size_t max_requests = 10000)
...
template <typename T> 
bool ThreadPool<T>::append_req(T *request)
{
    m_queuelocker.lock();
    // 请求数超出
    if (m_workqueue.size() >= m_max_requests) {
